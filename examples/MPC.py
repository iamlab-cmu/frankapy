...
class Perception():
    ...
    def rosmsg_shape_location(self, shape_type, timestamp, p0, z_stiffness_traj, i, prev_pose, grasp_location_rel):

        T_tag_world = self.detect_ar_world_pos(shape_class=shape_type)
        if T_tag_world == None:
            T_tag_world = prev_pose.copy()
        else:
            T_tag_world.translation += grasp_location_rel
            # T_tag_world.translation -= [0.0, 0.00, 0]
...
if __name__ == "__main__":
    ...
    fa = FrankaArm()
    grasp_location_rel = [0.0, -0.025, -0.12]
    ...
    prev_pose.translation += grasp_location_rel

    init_time = rospy.Time.now().to_time()
    for i in range(2, int(len(ts)/4)):
        timestamp = rospy.Time.now().to_time() - init_time
        
        traj_gen_proto_msg, ros_msg, prev_pose = percept.rosmsg_shape_location(Rectangle, timestamp, p0, z_stiffness_traj, i, prev_pose, grasp_location_rel)
