# TODO: need to add code to load GP reward model/retrain from prior collected data if starting from new epoch/sample
...
if __name__ == "__main__":
    ...
    # Initialize Gaussian policy params (DMP weights) - mean and sigma
    initial_wts, initial_mu, initial_sigma, S, control_type_z_axis = agent.initialize_gaussian_policy(args.cut_type, args.food_type, args.dmp_wt_sampling_var, args.start_from_previous, \
    ...
    for epoch in range(args.starting_epoch_num, args.num_epochs):
        ...
        if epoch!=0:
            ...
            # plot updated policy mean trajectory to visualize
            plot_updated_policy_mean_traject(work_dir, args.cut_typ, dmp_wts_file, epoch, args.dmp_traject_time, control_type_z_axis, init_dmp_info_dict,\
        ...
        np.savez(work_dir + '/' + 'GP_reward_model_training_data_epoch_'+str(epoch) + '.npz', GP_training_data_x_all = GP_training_data_x_all, \
        ...
        np.save(work_dir + '/' + 'queried_samples_all.npy', np.array(queried_samples_all))
        ...
        np.save(work_dir + '/' + 'total_queried_samples_each_epoch.npy', np.array(total_queried_samples_each_epoch))
        import pdb; pdb.set_trace()
