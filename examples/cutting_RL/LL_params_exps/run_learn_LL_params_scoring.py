...
if __name__ == '__main__':
    ...
    parser.add_argument('--position_dmp_weights_file_path', '-w', type=str, default='/home/sony/raw_IL_trajects/Jan-2021/011321_scoring_potato_position_weights_zeroYZ.pkl')
    ...
    parser.add_argument('--data_savedir', '-d', type=str, default='/home/sony/Documents/cutting_RL_experiments/data/Jan-2021-LL-param-exps/scoring/tomato/')
    ...
    starting_position = RigidTransform(rotation=knife_orientation, \
        translation=np.array([0.510, 0.111, 0.13]), #z=0.05
    ...
    if args.start_from_previous: # load previous data collected and start from updated policy and/or sample/epoch        
        ...
        initial_wts = np.array(init_dmp_info_dict['weights'])
        f_initial = -10 # TODO: maybe update?
        cart_pitch_stiffness_initial = 200 #20 
    ...
    for epoch in range(args.starting_epoch_num, args.num_epochs):
        ...
        # if not os.path.isfile(os.path.join(work_dir, 'policy_mean_each_epoch.npy')):
        #     np.save(os.path.join(work_dir, 'policy_mean_each_epoch.npy'),np.array(mean_params_each_epoch))
        #     np.save(os.path.join(work_dir, 'policy_cov_each_epoch.npy'),np.array(cov_each_epoch))
        # else: 
        #     import pdb; pdb.set_trace()
        #     old_pol_means = np.load(os.path.join(work_dir, 'policy_mean_each_epoch.npy'))
        #     old_pol_covs = np.load(os.path.join(work_dir, 'policy_cov_each_epoch.npy'))

        #     combined_pol_means = np.concatenate((old_pol_means, np.array(mean_params_each_epoch), axis = 0))
        #     combined_pol_covs = np.concatenate((old_pol_covs, np.array(cov_each_epoch), axis = 0))
        #     np.save(os.path.join(work_dir, 'policy_mean_each_epoch.npy'),np.array(combined_pol_means))
        #     np.save(os.path.join(work_dir, 'policy_cov_each_epoch.npy'),np.array(combined_pol_covs))
        # import pdb; pdb.set_trace()
